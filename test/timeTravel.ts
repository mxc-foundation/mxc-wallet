...
contract('Time Travel tools', () => {
  ...
  it('The time on the blockchain should be forwarded by two minutes', async () => {
    await mineBlock(web3)
    const timeBeforeTimeTravel = await readTimeFromChain(web3)
    await timeTravel(2 * 60, web3)
    const timeAfterTimeTravel = await readTimeFromChain(web3)
    assert.isTrue(
      Math.abs(timeAfterTimeTravel - timeBeforeTimeTravel - 120) <= 1
    )
  })
})
